...
void Mahony::begin(float sampleFrequency, float two_kp) {
...
Quaternion Quaternion::inverse() const { return {w, -x, -y, -z}; }
...
Vector3f Quaternion::rotate(const Vector3f &vec) const {
  Quaternion v(0.0, vec.x, vec.y, vec.z);
  Quaternion r = (*this) * v * this->inverse();
  return Vector3f{r.x, r.y, r.z};
}

void Quaternion::normalize() {
  float recip_norm = Mahony::invSqrt(w * w + x * x + y * y + z * z);
  w *= recip_norm;
  x *= recip_norm;
  y *= recip_norm;
  z *= recip_norm;
